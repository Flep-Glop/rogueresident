...
import { Node, NodeState, MapViewState } from '../types/map';
import Image from 'next/image';

/**
 * GameMap component - An enhanced map with Supergiant-inspired visual design.
 * 
 * Features:
 * - Asymmetric character portraits that break container boundaries
 * - Progressive node visibility with depth effects
 * - Animated connection pathways with contextual styling
 * - Micro-interactions for all interactive elements
 */
export default function GameMap() {
  // Get state from our game store
  const { 
    currentNodeId, 
    setCurrentNode, 
    completedNodeIds, 
  ...
  // Center map on active node when it changes with smooth animation
  useEffect(() => {
  ...
  // Handle mouse move for dragging with inertia effect
  ...
  const handleNodeSelect = (nodeId: string, e: MouseEvent) => {
    // Skip if this node is not accessible yet
    if (!isNodeAccessible(nodeId)) return;
    
    // Save the click position for ripple effect
    ...
    setTimeout(() => setRippleEffect({x: 0, y: 0, active: false}), 600);
    ...
    // Get node type for contextual flash effect
    const node = nodes.find(n => n.id === nodeId);
    const nodeType = node?.type || 'clinical';
    
    // Color varies by node type
    if (flashScreen) {
      const color = nodeType === 'boss' || nodeType === 'boss-ionix' ? 'red' : 
  ...
  // Get node color based on type with expanded color palette
  ...
  // Get node glow effect for active and hovered states
  const getNodeGlowEffect = (node: Node, isActive: boolean, isHovered: boolean): string => {
    if (!isActive && !isHovered) return 'none';
    
    const intensity = isActive ? '15px' : '8px';
    
    if (node.type === 'boss' || node.type === 'boss-ionix') 
      return `0 0 ${intensity} rgba(204, 77, 77, 0.5)`;
      
    if (node.type === 'qualification')
      return `0 0 ${intensity} rgba(214, 183, 64, 0.4)`;
    
    switch (node.character) {
      case 'kapoor': return `0 0 ${intensity} rgba(78, 131, 189, 0.4)`;
      case 'quinn': return `0 0 ${intensity} rgba(44, 146, 135, 0.4)`;
      case 'jesse': return `0 0 ${intensity} rgba(90, 105, 120, 0.4)`;
      case 'garcia': return `0 0 ${intensity} rgba(60, 138, 117, 0.4)`;
      default: return `0 0 ${intensity} rgba(78, 131, 189, 0.4)`;
    }
  };
  
  // Get connection visual style - enhanced with directional flow and contextual styling
  const getConnectionStyle = (sourceId: string, targetId: string) => {
    ...
    let pathWidth = 2;
    let pathOpacity = 0.3; // Lower base opacity
    let pathClass = '';
    let glowEffect = 'none';
    
    // Source is completed, target is accessible = highlighted path
    if (sourceCompleted && targetAccessible) {
      pathColor = 'rgba(52, 211, 153, 0.9)';
      pathWidth = 3;
      pathOpacity = 0.9;
      pathClass = 'animate-pulse-path';
      glowEffect = 'drop-shadow(0 0 3px rgba(52, 211, 153, 0.5))';
    } 
    // Source is completed, but target not accessible (requires other path)
    else if (sourceCompleted) {
      pathColor = 'rgba(52, 211, 153, 0.6)';
      pathWidth = 2;
      pathOpacity = 0.7;
    } 
    // Source is active, target is accessible = potential path
    else if (sourceState === 'active' && targetState === 'accessible') {
      pathColor = 'rgba(79, 107, 187, 0.8)';
      pathWidth = 2.5;
      pathOpacity = 0.8;
      pathClass = 'animate-pulse-path-subtle';
      glowEffect = 'drop-shadow(0 0 2px rgba(79, 107, 187, 0.3))';
    } 
    // Future nodes have dim paths
    else if (sourceState === 'future' || targetState === 'future') {
      pathOpacity = 0.2;
    } 
    // Locked nodes have nearly invisible paths
  ...
  // Render the connections between nodes with enhanced visuals
  ...
  const renderNode = (node: Node) => {
    ...
    // Runtime flags for these states
    ...
    // Skip rendering nodes that are completely locked and not connected to visible nodes
    ...
    // Node size varies by state - enhanced for more dramatic difference
    const baseWidth = 260;
    const baseHeight = 68;
    const width = isActive || isInteractiveAndHovered ? baseWidth * 1.15 : baseWidth;
    const height = isActive || isInteractiveAndHovered ? baseHeight * 1.15 : baseHeight;
    
    // Visual effects based on node state
    const nodeOpacity = isNodeLocked ? 0.15 : 
                         isNodeFuture ? 0.4 : 
                         isCompleted ? 0.9 : 1;
    
    const nodeBlur = isNodeLocked ? 'blur(2px)' : 
                     isNodeFuture ? 'blur(1px)' : 'none';
    
    const nodeGrayscale = isNodeLocked || isNodeFuture ? 'grayscale(1)' : 'grayscale(0)';
    
    // Create a unique node ID for DOM references
    const nodeId = `node-${node.id}`;
    
    // Classes for state
    let nodeClasses = 'absolute transition-all duration-300 transform node-container';
    
    // Visual variations by state
    switch (state) {
      case 'active':
        nodeClasses += ' z-30 shadow-pixel-lg brightness-110 scale-105';
        break;
      case 'completed':
        nodeClasses += ' z-20 shadow-pixel';
        break;
      case 'accessible':
        nodeClasses += ' hover:scale-105 z-10 shadow-pixel cursor-pointer node-interactive';
        break;
      case 'future':
        nodeClasses += ' z-0';
        break;
    ...
    // Character portrait has higher z-index to break out of node container
    ...
    return (
      <div 
        ...
        onClick={(e) => isInteractive && handleNodeSelect(node.id, e)}
        onMouseEnter={() => {
        ...
        style={{ 
          ...
          opacity: nodeOpacity,
          filter: `${nodeBlur} ${nodeGrayscale}`,
          // Contextual glow effect
          boxShadow: getNodeGlowEffect(node, isActive, isHovered)
        }}
      >
        {/* Ripple effect on node selection */}
        {rippleEffect.active && node.id === activeNode && (
          <div className="node-select-ripple"></div>
        )}
        ...
        <div className={`
          ...
          ${isActive || isInteractiveAndHovered ? 'scale-110' : ''}
        `} style={{ zIndex: portraitZIndex }}>
          <div className={`
            ...
            ${isInteractiveAndHovered && !isActive ? 'ring-1 ring-white ring-opacity-30' : ''}
            transition-all duration-300
          `} style={{ borderColor: getNodeColor(node) }}>
            {!isNodeLocked && (
              <Image
                ...
                fill
                className={`object-cover ${isNodeLocked ? 'opacity-0' : ''} scale-110 origin-top`}
              />
            )}
          </div>
        </div>
        ...
        {/* Node content */}
        <div className="pl-16 pr-3 py-2 h-full flex flex-col justify-center">
          {/* Node title with dynamic text size */}
          {!isNodeLocked ? (
            <PixelText className={`
              ${isActive || isInteractiveAndHovered ? 'text-lg' : 'text-base'} 
          ...
          {/* Node description - visible on active/hover, truncated otherwise */}
          {!isNodeLocked && node.description && (
            <PixelText className={`
              text-xs text-gray-300 transition-all duration-300
              ${isActive || isInteractiveAndHovered ? 'line-clamp-2 mt-1 opacity-100' : 'line-clamp-1 opacity-80'}
            `}>
              {node.description}
            </PixelText>
          )}
          ...
          {!isNodeLocked && (
            <div className="absolute top-2 right-2 text-xs px-1.5 py-0.5 rounded-sm bg-black bg-opacity-50">
  ...
  const handleEndDay = () => {
    ...
    // Zoom out effect on the entire map container
    if (mapContainerRef.current) {
      mapContainerRef.current.animate(
        [
          { transform: 'scale(1)', filter: 'brightness(1)' },
          { transform: 'scale(0.98)', filter: 'brightness(1.5)' }
        ],
  ...
  // Focus the view on active node with smooth animation
  ...
  return (
    <div className="h-full w-full flex flex-col relative bg-background overflow-hidden">
      ...
      <div className="text-center py-4 bg-background/90 backdrop-blur-sm sticky top-0 z-50">
        <PixelText className="text-3xl text-white font-pixel-heading glow-text">
      ...
      >
        ...
        >
          {/* Starfield parallax background - subtle depth effect */}
          <div 
            className="absolute inset-0" 
            style={{
              backgroundImage: 'radial-gradient(circle, rgba(255, 255, 255, 0.03) 1px, transparent 1px)',
              backgroundSize: '40px 40px',
              transform: `translate(${-viewState.offsetX * 0.1}px, ${-viewState.offsetY * 0.1}px)`,
              opacity: 0.5
            }}
          ></div>
        ...
        {/* Mini-map overlay */}
        <div className="absolute bottom-4 right-4 w-48 h-36 bg-background/80 rounded-md border border-gray-700 z-40 pixel-borders-thin overflow-hidden">
          ...
          <div className="relative w-full h-full p-2">
            ...
            {nodes.map((node: Node) => {
              ...
              return (
                <div 
                  key={`minimap-${node.id}`}
                  className={`absolute rounded-full transform -translate-x-1/2 -translate-y-1/2 ${state === 'active' ? 'animate-pulse-slow' : ''}`}
                  style={{
                    left: `${node.position.x}%`,
                    top: `${node.position.y}%`,
                    ...
                    boxShadow: state === 'active' ? '0 0 5px currentColor' : 'none'
                  }}
                />
              );
            })}
        ...
        {/* Map controls overlay with enhanced visuals */}
        <div className="absolute top-4 right-4 flex flex-col space-y-2 z-40">
          <button 
            className="w-8 h-8 bg-surface-dark hover:bg-surface flex items-center justify-center rounded-md pixel-borders-thin enhanced-button"
          ...
          <button 
            className="w-8 h-8 bg-surface-dark hover:bg-surface flex items-center justify-center rounded-md pixel-borders-thin enhanced-button group"
            onClick={focusOnActiveNode}
            aria-label="Focus on active node"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white group-hover:animate-pulse-slow" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-surface/90 backdrop-blur-sm pixel-borders-thin px-4 py-2 flex items-center space-x-4 z-50">
        ...
        <PixelButton
          className="bg-surface hover:bg-clinical text-text-primary px-4 py-2 enhanced-button relative overflow-hidden group"
      ...
      {/* Enhanced map legend */}
      <div className="fixed bottom-20 right-4 bg-surface/80 backdrop-blur-sm p-3 text-xs pixel-borders-thin z-40">
